...
import AITM_LevelCodeLabel from '@salesforce/label/c.AITM_LevelCodeLabel';
...
export default class AITM_TenderLocationRecordEditPage extends LightningElement {
        ...
        if(this.tfWrapperRelatedFields){
            ...
            for (let key in data) {
                ...
                if(key == AITM_CreditDaysCodeLabel && data[key] == COLORCODE)    
                    ...
                    .classList
                    .add('redCSS');
                if(key == AITM_LevelCodeLabel && data[key] == COLORCODE)    
                    this.template.querySelector('.applyRedForLevel')
                    .classList
                    .add('redCSS');
            }
        }        
    ...
    disableToggleSpinner(){
        const togglespinner = new CustomEvent
                                            (
                                                'togglespinner',
                                                    {   
                                                        detail: ''
    ...
    get level(){ 
        return (this.tfWrapperRelatedFields && this.tfWrapperRelatedFields.record.AITM_Tender_Location__r.AITM_Service_Level__c) ? this.tfWrapperRelatedFields.record.AITM_Tender_Location__r.AITM_Service_Level__c : '';
    }
