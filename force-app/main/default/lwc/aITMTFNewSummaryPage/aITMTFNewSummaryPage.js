...
import NOTES_FIELD from '@salesforce/schema/AITM_Tender_Location_Line_Item__c.AITM_Additional_Notes_Taxes_and_Fees__c';
    ...
    handleReset() {
        ...
        if (inputFields) {
            inputFields.forEach(field => {
                field.reset();
                if(field.name == 'AITM_Taxes_Fees__c')
                    this.taxesNotes = field.value;
            });
        }
    }

    @wire(getRecord, { recordId: '$selLineId', fields: NOTES_FIELD })
    wiredLine({ data }) {
        if (data){
            this.template.querySelector('.textAreaCSS').value = getFieldValue(data, NOTES_FIELD);
        }
    }
}