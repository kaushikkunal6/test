({
  doInit: function(component,event){
    this.getTableData(component,event,true);
    this.getPicklistValues(component,"picklist-location-status","AITM_Tender_Location__c","AITM_Status__c");
    this.getPicklistValuesForCurrencies(component, 'AITM_Tender_Location_Line_Item__c', 'AITM_Currency__c');
    this.getPicklistValuesForUOM(component, 'AITM_Tender_Location_Line_Item__c', 'AITM_Unit_Of_Measure__c');
  },
    ...
    getOldTaxFeeFlag: function(component, event){
        ...
        var recordId = component.get("v.tenderLocId");
  ...
  getTableData: function(component,event,getTableMetadata) {
	var recordId = component.get("v.tenderLocId");
    ...
    action.setCallback(this, function(actionResult) {
      ...
      if (state === 'SUCCESS') {
        ...
        component.set("v.tableSize",(result.lineItems.length-1));
        ...
        if(component.get("v.calledViaDoInit")){
            var holdingFirstLineId;
            var holdingFirstCustomerId = result.acc[0].Id;
            for (let i = 0; i < result.lineItems.length; i += 1) {
            	if (result.lineItems[i].AITM_Account__c == holdingFirstCustomerId && !holdingFirstLineId)
                    holdingFirstLineId = result.lineItems[i].Id;
            }
            component.set("v.selectedLineId",holdingFirstLineId);
        	this.handleSplitEvent(component, event, holdingFirstLineId,false);
            component.set("v.calledViaDoInit", false);
        }else{
            this.handleSplitEvent(component, event, component.get("v.selectedLineId"),component.get("v.handlePartialRefresh"));
        }
      } 
  ...
  getTableMetadata: function(component){
    var action = component.get("c.getNewUITableMetadata");
    action.setCallback(this, function(response) {
      var state = response.getState();
        if (state === "SUCCESS") {
          var returnValue = response.getReturnValue();
...
//component.find("straddle-taxes-fees").set("v.disabled", true);
  ...
  changeTenderLocationStatus: function(component){
    var tenderLocation = component.get("v.tenderLocation");
    var statusComponent = component.find("picklist-location-status");
    var status = (typeof statusComponent != 'undefined' ? statusComponent.get("v.value") : '');
  ...
  getPicklistValues: function(component, statusElementId, targetObject, targetFieldName){
    var locationStatusElement = component.find(statusElementId);
    var action = component.get("c.getPickValues");
    var value = (typeof locationStatusElement!= 'undefined'?locationStatusElement.get('v.value'):'');
    action.setParams({
    ...
    action.setCallback(this, function(response) {
        ...
        if (state === "SUCCESS") {
            ...
            if (returnValue) {
                if(typeof locationStatusElement!= 'undefined') {
                	locationStatusElement.set('v.options', options);
                }
            }
        }
    ...
    getPicklistValuesForCurrencies: function(component, targetObject, targetFieldName){
    var action = component.get("c.getPickValues");
    ...
    action.setCallback(this, function(response) {
        ...
        if (state === "SUCCESS") {
            ...
            options.push("None");
            for(var i = 0; i<returnValue.length; i++){
                options.push(returnValue[i]);
            }
            if (returnValue) {
                component.set('v.currencyOptions', options);
            }
        }
    ...
    getPicklistValuesForUOM: function(component, targetObject, targetFieldName){
    var action = component.get("c.getPickValues");
    var targetObject = targetObject;
    var targetFieldName = targetFieldName;
    action.setParams({
    ...
    action.setCallback(this, function(response) {
        ...
        if (state === "SUCCESS") {
            ...
            if (returnValue) {
                component.set('v.UOMOptions', options);
            }
        }
      ...
      if (state === "SUCCESS") {
        if (response.getReturnValue() != '') {
          ...
          let checkBoxState = event.getSource().get('v.value');
           component.find("cloneenable").set("v.disabled", !checkBoxState);
        component.find("deleteenable").set("v.disabled", !checkBoxState);
        $A.get('e.force:refreshView').fire();
      }
    });
    $A.enqueueAction(action);
  ...
  cloneSelectedHelper: function(component,event,helper) {
    ...
    let cloneIds = component.get("v.selectedCheckBoxes");     
    var action = component.get('c.cloneRecords');
     action.setParams({
      "cloneRecordId": cloneIds  
    });
  ...
  hideSpinner : function(component) {
    ...
    $A.util.addClass(spinner, "slds-hide");
      ...
      handleSplitEvent : function(component, event , getSelectRecord, partialRefresh) { 
          var compEvent = component.getEvent("splitScreenEvent");
          component.set("v.handlePartialRefresh", true);
          compEvent.setParams({
              "selectedLineId" : getSelectRecord,
              "hasPartialRefresh" : partialRefresh,
          });  
          compEvent.fire();
	},
})